/**
 * SPC-003 FR-001, FR-002: Reference slot resolution + priority pruning engine.
 * Pure module — no DOM, no Chrome APIs, deterministic.
 *
 * Source of truth: extraction/EXTRACTION.md §5 as corrected by
 * extraction/TS_SOURCE_SUPPLEMENT.md (D7, D8, D13, V19-V22, N3-N5) —
 * reference apps' unminified shared-core `hooks/usePromptState.ts:355-600`.
 *
 ...
 *   edit = global first, then characters, scene, anonymous (D7).
 ...
 *   survivors keep their original construction order (V19, selectReferenceCandidates).
 * - Overflow is retained, never silently dropped (N5).
 * - In edit mode the global ref stays numbered in the instruction text (image_1);
 *   only the upload payload drops it — payload selection is the caller's concern (D8).
 */
